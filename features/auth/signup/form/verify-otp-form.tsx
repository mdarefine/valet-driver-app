...
import { OTPRequest, otpSchema } from '../../forget/types'
import useSession from '@/store/useSession'
import Toast from 'react-native-toast-message'
import useCountdown from '@/hooks/useCountdown'
const VerifyOtpForm = () => {
  const router = useRouter()
  const { updateToken, updateIsLoggedIn } = useSession()
  ...
  const onSubmit = async (data: OTPRequest) => {
    await verifyOtp({
      ...
      if (result?.data?.verifyOtp?.data?.jwt) {
        updateToken(result?.data?.verifyOtp?.data?.jwt)

        router.push({
          pathname: '/(auth)/(sign-up)/sign-up-complete',
        })
      }
    })
  }
